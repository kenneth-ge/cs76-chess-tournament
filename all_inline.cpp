...
pair<Move, int> max_value(ChessRules &board, int depth, int alpha, int beta){
    auto [stop, eval] = cutoff_test(board, depth);
    ...
    for(auto x: moves){
        ...
        auto [move, curr_eval_value] = min_value(board, depth+1, alpha, beta);
    ...
    if(best_move == nullptr){
        return {nullptr, evaluate(board)};
    }
}

pair<Move, int> min_value(ChessRules &board, int depth, int alpha, int beta){
    auto [stop, eval] = cutoff_test(board, depth);
    ...
    for(auto x: moves){
    	cr.PlayMove(x);
        display_position(cr, "Possible move");
        auto [move, curr_eval_value] = max_value(board, depth+1, alpha, beta);
        if(curr_eval_value < best_eval_value) {
            best_eval_value = curr_eval_value;
            best_move = move;
        }
